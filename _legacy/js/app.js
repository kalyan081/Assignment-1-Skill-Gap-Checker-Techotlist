 ...
 *
 * Wires up the Analyze button, renders results, manages history,
 * and handles save/share actions.
 */
...
const App = (() => {
  ...
  function _renderBadges(container, skills, type) {
    ...
    if (!skills || skills.length === 0) {
      container.innerHTML = `<span class="empty-state">${
        type === 'match' ? 'No matching skills found' : 'No gaps — great match!'
      }</span>`;
      return;
    }
